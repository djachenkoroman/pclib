...
def get3dviz(xs, ys, zs, labels, title, frac=1):
    cdict = {
        0: 'red',
        1: 'orange',
        2: 'green',
    ...
    xyz_labels = np.vstack([xs, ys, zs, labels])
    idx = np.random.randint(len(xyz_labels[0, :]), size=int(len(xyz_labels[0, :]) * frac))
    xs, ys, zs, labels = xyz_labels[0, :][idx], xyz_labels[1, :][idx], xyz_labels[2, :][idx], xyz_labels[3, :][idx]
    fig = px.scatter_3d(x=xs,y=ys,z=zs,color=[cdict[label] for label in labels],color_discrete_map="identity",title=title, size_max=18)
    return fig
