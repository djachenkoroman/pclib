...
class STNkd(nn.Module):
    ...
    def forward(self, x):
        ...
        iden = Variable(torch.from_numpy(np.eye(self.channels).flatten().astype(np.float32))).view(1, self.channels * self.channels).repeat(batchsize, 1)
        if x.is_cuda:
            iden = iden.cuda()
        x = x + iden
        x = x.view(-1, self.channels, self.channels)
        return x

class PointNetfeat(nn.Module):
    def __init__(self, channels=6, global_feat = True, feature_transform = False):
        super(PointNetfeat, self).__init__()
        self.stn = STNkd(channels=channels)
        self.conv1 = torch.nn.Conv1d(channels, 64, 1)
        self.conv2 = torch.nn.Conv1d(64, 128, 1)
        self.conv3 = torch.nn.Conv1d(128, 1024, 1)
        self.bn1 = nn.BatchNorm1d(64)
